...
func _show_error_or_success(errs: PackedStringArray, success_message: String, output_dir: String):
	if errs.size() > 0:
		...
		# On web: zip the output and trigger a browser download instead of opening a folder
		var open_label := "Download ZIP" if GDREWebCompat.is_web() else "Open Folder"
		popup_confirm_box(success_message, "Success", func(): GDREWebCompat.open_or_download_folder(output_dir), func(): pass, open_label, "OK")
